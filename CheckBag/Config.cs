...
namespace CheckBag
{
    public class Config
    {
        public string 配置说明 = "检测间隔为xx秒，封禁时长为xx分钟（只封UUID），警告次数达标后封禁，物品名称可不写。";
        public string 物品查询 = "https://terraria.wiki.gg/zh/wiki/Item_IDs";
        public int 检测间隔 = 5;  // 秒
        public int 封禁时长 = 10; // 分钟
        public int 警告次数 = 15;  // 次
        public List<ItemData> 全时期 = new();
        public List<ItemData> 哥布林前 = new();
        public List<ItemData> 史王前 = new();
        public List<ItemData> 克眼前 = new();
        public List<ItemData> 鹿角怪前 = new();
        public List<ItemData> 世吞克脑前 = new();
        public List<ItemData> 蜂王前 = new();
        public List<ItemData> 骷髅王前 = new();
        public List<ItemData> 肉前 = new();
        public List<ItemData> 皇后前 = new();
        ...
        public List<ItemData> 拜月前 = new();
        public List<ItemData> 月前 = new();
        ...
        public List<ItemData> Current()
        {
            ...
            if (!NPC.downedSlimeKing) list.AddRange(史王前);
            if (!NPC.downedGoblins) list.AddRange(哥布林前);
            if (!NPC.downedBoss1) list.AddRange(克眼前);
            if (!NPC.downedDeerclops) list.AddRange(鹿角怪前);
            if (!NPC.downedBoss2) list.AddRange(世吞克脑前);
            if (!NPC.downedQueenBee) list.AddRange(蜂王前);
            if (!NPC.downedBoss3) list.AddRange(骷髅王前);
            if (!Main.hardMode) list.AddRange(肉前);
            if (!NPC.downedQueenSlime) list.AddRange(皇后前);
            ...
            if (!NPC.downedAncientCultist) list.AddRange(拜月前);
            if (!NPC.downedMoonlord) list.AddRange(月前);
            return list;
        }
